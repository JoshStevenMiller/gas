...
st.set_page_config(page_title="Goal Attainment Scaling (GAS) Web Tool", layout="centered")

st.title("🎯 Goal Attainment Scaling (GAS)")
st.markdown("Enter up to **five goals**, assign a relative **weight** (0.2 to 1.0), and select a **score** from -2 to +2. Then download your results as a PDF.")
...
with st.form("gas_form"):
    ...
    for i in range(num_goals):
        ...
        weight = st.number_input(f"Weight (0.2–1.0) for goal {i+1}", min_value=0.2, max_value=1.0, step=0.1, key=f"weight_{i}")
        score = st.selectbox(f"Score (-2 to +2) for goal {i+1}", score_options, key=f"score_{i}")
        goals.append(goal)
        weights.append(weight)
        scores.append(score)
    submitted = st.form_submit_button("✨ Calculate and Generate PDF")
